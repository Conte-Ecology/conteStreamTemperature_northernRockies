...
e <- read.table("dataLocal/CCE_DAILY_110414.csv", sep = ",", header = T)
str(e)
...
e <- e %>%
  mutate(temp = stream_mn, airTemp = TmeanD, site = SiteID, HUC8 = as.character(HUC), dOY = strptime(e$date, "%m/%d/%Y")$yday+1) # I just call it HUC8 to match northeast code for now
...
for ( i in 1:nrow(siteYearCombos)){
 ...
 # print(i)
  ...
  tempBreaks <- data.frame( year  = as.numeric  (siteYearCombos$year[i]),
                           ...
                           # HUC12 = as.character(unique(e$HUC8[which(e$site == siteYearCombos$site[i])])), # Made this HUC8 just to work
                            HUC8  = as.character(unique(e$HUC8 [which(e$site == siteYearCombos$site[i])])),
                            #HUC4  = as.character(unique(e$HUC8 [which(e$site == siteYearCombos$site[i])])), #Temporary hack made = to HUC8 just to run
                            quantileLo = NA,
                            quantileHi = NA)
      ...
      for (i in 1:(200)){
        ...
        for (ii in 2:numForwardSpring ){
          ...
          if( (i+ii-2) %in% eYear$dOY ) {
			  	  runsSpring[ i,ii ] <- 1*all((eYear$movingMean[ eYear$dOY == (i+ii-2) ] >= lo) & (eYear$movingMean[ eYear$dOY == (i+ii-2) ] <= hi))
    ...
    # Only calculate if it is a complete season
      ...
      for (i in  max(eYear$dOY):stopLoop){
        ...
        # From the current day, loop backward through the numForward range to determined which days are in sync
          ...
          #     mean is outside of the range, it gets assigned a zero.
				  if( (i-ii+2) %in% eYear$dOY ) { 
					  runsFall[ i,ii ] <- 1*all((eYear$movingMean[ eYear$dOY == (i-ii+2) ] >= lo) & (eYear$movingMean[ eYear$dOY == (i-ii+2) ] <= hi))
...
meanBP  <- ddply( breaks, .(), summarise, meanSpringBP  = mean(springBP, na.rm=T), meanFallBP  = mean(fallBP ,na.rm=T) )
...
# -----------------
# Spring
remainingBP <- which(is.na(breaks$finalSpringBP))
breaks$finalSpringBP [ remainingBP ] <- trunc(meanBP$meanSpringBP)
breaks$sourceSpringBP[ remainingBP ] <- 'overall mean'

# Fall
remainingBP <- which(is.na(breaks$finalFallBP))
breaks$finalFallBP [ remainingBP ] <- trunc(meanBP$meanFallBP)
breaks$sourceFallBP[ remainingBP ] <- 'overall mean'
